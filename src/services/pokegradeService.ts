...
class PokegradeService {
  ...
  parseQRData(qrData: string): { pokegrade_id: string } | null {
    try {
      ...
      if (
        parsed.pokegrade_id &&
        this.validatePokegradeId(parsed.pokegrade_id)
      ) {
        return parsed;
      }
      return null;
  ...
  private async verifyCardFallback(
    pokegradeId: string
  ): Promise<PokegradeResponse> {
    try {
      const response = await fetch(`${this.apiUrl}/lookup/${pokegradeId}`, {
        method: "GET",
        headers: {
  ...
  private getMockData(pokegradeId: string): PokegradeResponse {
    ...
    if (pokegradeId === "PG-20250906-002" || pokegradeId.includes("001")) {
      return {
        success: true,
        card: {
